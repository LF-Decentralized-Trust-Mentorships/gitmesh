...
async function getQuestions(
  ...
  try {
    ...
    // Try to get an access token from Nango, but continue without it if not available
    // Stack Overflow API works without authentication using just the API key
    let accessToken = null
    try {
      accessToken = await getNangoToken(input.nangoId, 'stackexchange', ctx)
    } catch (err) {
      ctx.log.warn({ err }, 'Could not get Nango token for Stack Overflow, continuing without authentication')
    }
    ...
    // Only add access_token if we have one
    if (accessToken) {

...
class RawQueryParser {
    static parseFilters(filters, columnMap, jsonColumnInfos, params) {
        ...
        for (const key of keys) {
            ...
            else if (key === types_2.Operator.NOT) {
    ...
    static parseJsonColumnCondition(property, filters, params) {
        ...
        if (parts.length > 0) {
            ...
            const attributeInfo = (0, array_1.singleOrDefault)(property.info.attributeInfos, (a) => a.name === attribute);
        ...
        if (Array.isArray(filters)) {
            operator = types_2.Operator.CONTAINS;
            value = filters;
        }
        else {
        ...
        if (operator === types_2.Operator.BETWEEN || operator === types_2.Operator.NOT_BETWEEN) {
            // we need two values
        ...
        if (operator === types_2.Operator.CONTAINS || operator === types_2.Operator.OVERLAP) {
        ...
        if (operator === types_2.Operator.IN || operator === types_2.Operator.NOT_IN) {
        ...
        const paramName = this.getJsonParamName(property.info.property, parts, params);
        if (operator === types_2.Operator.LIKE || operator === types_2.Operator.NOT_LIKE) {
    ...
    static parseColumnCondition(key, column, filters, params) {
        ...
        const paramName = this.getParamName(key, params);
        if (operator === types_2.Operator.EQUAL &&
            (value === null || (typeof value === 'string' && value.toLowerCase() === 'null'))) {
            params[paramName] = null;
            return `(${column} is :${paramName})`;
        }
        if (operator === types_2.Operator.NOT_EQUAL &&
            (value === null || (typeof value === 'string' && value.toLowerCase() === 'null'))) {
            params[paramName] = null;
            return `(${column} is not :${paramName})`;
        }
        if (operator === types_2.Operator.LIKE ||
            operator === types_2.Operator.NOT_LIKE ||
            operator === types_2.Operator.TEXT_CONTAINS ||
            operator === types_2.Operator.NOT_TEXT_CONTAINS) {
            params[paramName] = `%${value}%`;
        }
        else {
            params[paramName] = value;
    ...
    static getJsonColumnInfo(column, jsonColumnInfos) {
        ...
        const info = (0, array_1.singleOrDefault)(jsonColumnInfos, (jsonColumnInfo) => jsonColumnInfo.property === actualProperty);
        if (info) {
            return {
                info,
                parts,
    ...
    static getOperator(operator, json = false) {
        switch (operator) {
            case types_2.Operator.GREATER_THAN:
                return '>';
            case types_2.Operator.GREATER_THAN_OR_EQUAL:
                return '>=';
            case types_2.Operator.LESS_THAN:
                return '<';
            case types_2.Operator.LESS_THAN_OR_EQUAL:
                return '<=';
            case types_2.Operator.NOT_EQUAL:
            case types_2.Operator.NOT:
                return '<>';
            case types_2.Operator.EQUAL:
                return '=';
            case types_2.Operator.LIKE:
            case types_2.Operator.TEXT_CONTAINS:
                return 'ilike';
            case types_2.Operator.NOT_LIKE:
            case types_2.Operator.NOT_TEXT_CONTAINS:
                return 'not ilike';
            case types_2.Operator.AND:
                return 'and';
            case types_2.Operator.OR:
                return 'or';
            case types_2.Operator.IN:
                return 'in';
            case types_2.Operator.NOT_IN:
                return 'not in';
            case types_2.Operator.BETWEEN:
                return 'between';
            case types_2.Operator.NOT_BETWEEN:
                return 'not between';
            case types_2.Operator.OVERLAP:
                if (json) {
                    return '?|';
                }
                return '&&';
            case types_2.Operator.CONTAINS:
                if (json) {
                    return '?&';
                }
                return '@>';

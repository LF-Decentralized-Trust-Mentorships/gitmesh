...
exports.OrganizationSyncService = void 0;
const organization_repo_1 = require("../repo/organization.repo");
const segment_repo_1 = require("../repo/segment.repo");
const common_1 = require("@gitmesh/common");
const logging_1 = require("@gitmesh/logging");
const types_1 = require("@gitmesh/types");
class OrganizationSyncService {
    constructor(store, openSearchService, parentLog, serviceConfig) {
        this.openSearchService = openSearchService;
        this.log = (0, logging_1.getChildLogger)('organization-sync-service', parentLog);
        this.serviceConfig = serviceConfig;
        this.orgRepo = new organization_repo_1.OrganizationRepository(store, this.log);
    ...
    async getAllIndexedTenantIds(pageSize = 500, afterKey) {
        ...
        const results = await this.openSearchService.search(types_1.OpenSearchIndex.ORGANIZATIONS, undefined, {
    ...
    async cleanupOrganizationIndex(tenantId) {
        ...
        const pageSize = 500;
        let lastCreatedAt;
        let results = (await this.openSearchService.search(types_1.OpenSearchIndex.ORGANIZATIONS, query, undefined, pageSize, sort, undefined, include));
        let processed = 0;
        while (results.length > 0) {
            // check every organization if they exists in the database and if not remove them from the index
            const dbIds = await this.orgRepo.checkOrganizationsExists(tenantId, results.map((r) => r._source.uuid_organizationId));
            ...
            if (toRemove.length > 0) {
                ...
                for (const id of toRemove) {
                    await this.openSearchService.removeFromIndex(id, types_1.OpenSearchIndex.ORGANIZATIONS);
                }
            }
            ...
            results = (await this.openSearchService.search(types_1.OpenSearchIndex.ORGANIZATIONS, query, undefined, pageSize, sort, lastCreatedAt, include));
        }
        this.log.warn({ tenantId }, `Processed total of ${processed} organizations while cleaning up tenant!`);
    }
    async removeOrganization(organizationId) {
        ...
        while (results.length > 0) {
            ...
            results = (await this.openSearchService.search(types_1.OpenSearchIndex.ORGANIZATIONS, query, undefined, pageSize, sort, lastJoinedAt, include));
        }
    }
    async syncTenantOrganizations(tenantId, batchSize = 200, syncCutoffTime) {
        ...
        await (0, logging_1.logExecutionTime)(async () => {
            let organizationIds = await this.orgRepo.getTenantOrganizationsForSync(tenantId, 1, batchSize, cutoffDate);
            while (organizationIds.length > 0) {
                const { organizationsSynced, documentsIndexed } = await this.syncOrganizations(organizationIds);
                organizationCount += organizationsSynced;
                docCount += documentsIndexed;
                this.log.info({ tenantId }, `Synced ${organizationCount} organizations with ${docCount} documents!`);
                organizationIds = await this.orgRepo.getTenantOrganizationsForSync(tenantId, 1, batchSize, cutoffDate);
            }
        }, this.log, 'sync-tenant-organizations');
        this.log.info({ tenantId }, `Synced total of ${organizationCount} organizations with ${docCount} documents!`);
    }
    async syncOrganizations(organizationIds) {
        ...
        if (organizations.length > 0) {
            ...
            let segmentInfos;
            if (isMultiSegment) {
                childSegmentIds = (0, common_1.distinct)(organizations.map((m) => m.segmentId));
                segmentInfos = await this.segmentRepo.getParentSegmentIds(childSegmentIds);
            }
            const grouped = (0, common_1.groupBy)(organizations, (m) => m.organizationId);
                ...
                if (isMultiSegment) {
                    for (const organization of organizationDocs) {
                        ...
                        for (const parentId of parentIds) {
                            const aggregated = OrganizationSyncService.aggregateData(organizationDocs, relevantSegmentInfos, parentId);
                        ...
                        for (const grandParentId of grandParentIds) {
                            const aggregated = OrganizationSyncService.aggregateData(organizationDocs, relevantSegmentInfos, undefined, grandParentId);
                            const prepared = OrganizationSyncService.prefixData(aggregated);
                            forSync.push({
                    ...
                    if (organizationDocs.length > 1) {
                        throw new Error('More than one organization found - this can not be the case in single segment edition!');
                    }
            ...
            await this.openSearchService.bulkIndex(types_1.OpenSearchIndex.ORGANIZATIONS, forSync);
    ...
    static aggregateData(segmentOrganizationss, segmentInfos, parentId, grandParentId) {
        ...
        const organizations = segmentOrganizationss.filter((m) => relevantSubchildIds.includes(m.segmentId));
        if (organizations.length === 0) {
        ...
        // gather only uniques
        organization.activeOn = (0, common_1.distinct)(organization.activeOn);
        organization.identities = (0, common_1.distinct)(organization.identities);
        return organization;
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
//# sourceMappingURL=organization.sync.service.js.map
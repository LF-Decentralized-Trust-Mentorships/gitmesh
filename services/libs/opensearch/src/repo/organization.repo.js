...
class OrganizationRepository extends database_1.RepositoryBase {
    constructor(dbStore, parentLog) {
        super(dbStore, parentLog);
    }
    async getOrganizationData(ids) {
    ...
    async checkOrganizationsExists(tenantId, orgIds) {
        const results = await this.db().any(`
      select id 
      from 
        organizations 
    ...
    async markSynced(orgIds) {
        await this.db().none(`update organizations set "searchSyncedAt" = now() where id in ($(orgIds:csv))`, {
            orgIds,
        });
    }
    async getTenantOrganizationsForSync(tenantId, page, perPage, cutoffDate) {
        const results = await this.db().any(`
        select o.id
        from organizations o

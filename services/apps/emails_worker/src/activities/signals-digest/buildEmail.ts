...
import { SignalsPostWithActions, SignalsRawPost } from '@gitmesh/types'
...
/*
fetchFromSignals is a Temporal activity that fetches the content to push inside
an email coming from Signals API.
*/
export async function fetchFromSignals(user: UserTenant): Promise<SignalsRawPost[]> {
  const feedSettings = user.settings.signals.feed
...
*/
export async function fetchFromDatabase(user: UserTenant): Promise<SignalsRawPost[]> {
  const feedSettings = user.settings.signals.emailDigest.feed || user.settings.signals.feed
  const actualdate = switchDate(feedSettings.publishedDate, 90)

  let posts: SignalsRawPost[]
  try {
    posts = await svc.postgres.reader.connection().query(
      `SELECT url, post thumbnail, platform
        FROM "signalsContents"
        WHERE "tenantId" = $1
        AND "postedAt" > $2;`,
...
based on posts previously fetched from Signals API and the database.
*/
export async function buildEmailContent(posts: Content): Promise<SignalsPostWithActions[]> {
  const interactedMap = {}
  ...
  for (const item of posts.fromSignals as SignalsRawPost[]) {
    const post = {
  ...
  const content = out.slice(0, 10).map((c: SignalsPostWithActions) => {

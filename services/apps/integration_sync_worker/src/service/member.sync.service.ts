...
import { Entity, HubspotSettings, IMember, OpenSearchIndex } from '@gitmesh/types'
    ...
    do {
      ...
      while (markedMembers.length > 0) {
        ...
        const translatedMembers: IMember[] = memberFromOpensearch.reduce((acc, member) => {
          const membersWithGitmeshFields = translator.translateObjectToGitmesh(member._source)
          acc.push(membersWithGitmeshFields)
          return acc
        }, [])

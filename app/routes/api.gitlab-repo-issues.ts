...
async function gitlabRepoIssuesLoader({ request }: { request: Request }) {
  try {
    ...
    // Fetch issues
    const url = `${gitlabUrl}/api/v4/projects/${encodeURIComponent(projectId)}/issues?${queryParams.toString()}`;
    ...
    const issues: GitLabIssue[] = await response.json();
...
export const loader = withSecurity(gitlabRepoIssuesLoader);
export const action = withSecurity(gitlabRepoIssuesLoader);

...
ADD COLUMN "settings" JSONB DEFAULT '{"isSignalsGuideDismissed": false, "isQuickstartGuideDismissed": false, "signals": { "onboarded": false } }';
...
UPDATE "tenantUsers"
SET "settings"= jsonb_set("settings", '{signals}', u."signalsSettings"::jsonb)
FROM "users" u
WHERE "tenantUsers"."userId" = u.id
and (u."signalsSettings"->>'onboarded')::boolean is true;

ALTER TABLE "users" DROP COLUMN "signalsSettings"
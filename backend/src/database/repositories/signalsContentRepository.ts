...
import { SignalsContent } from '@gitmesh/types'
...
export default class SignalsContentRepository {
  static async create(
    data: SignalsContent,
    options: IRepositoryOptions,
  ): Promise<SignalsContent> {
    const currentTenant = SequelizeRepository.getCurrentTenant(options)

    const record = await options.database.signalsContent.create({
    ...
    if (data.actions) {
      for (const action of data.actions) {
        await SignalsActionRepository.createActionForContent(action, record.id, options)
      }
    }
  ...
  static async findAndCountAll(
    ...
    let rows = await options.database.signalsContent.findAll({
      include,
    ...
    const count = (
      await options.database.signalsContent.count({
        include,
        ...(parsed.where ? { where: parsed.where } : {}),
        transaction: SequelizeRepository.getTransaction(options),
        distinct: true,
        group: ['signalsContent.id'],
      })
    ).length

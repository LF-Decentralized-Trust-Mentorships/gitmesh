...
import { PlatformType, SegmentStatus } from '@gitmesh/types'
...
describe('MemberRepository tests', () => {
  ...
  describe('findAndCountAll method', () => {
    ...
    it('is successfully finding and counting all members, and organisations [gitmesh.dev, pied piper]', async () => {
      ...
      const gitmeshOrg = await mockIRepositoryOptions.database.organization.create({
        displayName: 'gitmesh.dev',
        tenantId: mockIRepositoryOptions.currentTenant.id,
        segmentId: mockIRepositoryOptions.currentSegments[0].id,
      })
      ...
      await MemberRepository.create(
        {
          ...
          score: '6',
          joinedAt: new Date(),
          organizations: [gitmeshOrg.id, pp.id],
        },
        mockIRepositoryOptions,
      )
      await MemberRepository.create(
      ...
      const members = await MemberRepository.findAndCountAll(
        { filter: { organizations: [gitmeshOrg.id, pp.id] } },
        mockIRepositoryOptions,
      )
  ...
  describe('findAndCountAllv2 method', () => {
    ...
    it('is successfully finding and counting all members, and organisations [gitmesh.dev, pied piper]', async () => {
      ...
      const gitmeshOrg = await OrganizationRepository.create(
        {
          identities: [
            {
              name: 'gitmesh.dev',
  ...
  describe('update method', () => {
    ...
    it('Should successfully update member with given organizations', async () => {
      ...
      const org1 = await OrganizationRepository.create(
        {
          displayName: 'gitmesh.dev',
          identities: [{ name: 'gitmesh.dev', url: 'https://gitmesh.dev', platform: 'gitmesh' }],
        },
        mockIRepositoryOptions,
      )
      const org2 = await OrganizationRepository.create(
        {
          displayName: 'pied piper',
          identities: [{ name: 'pied piper', url: 'https://piedpiper.com', platform: 'gitmesh' }],
        },
        mockIRepositoryOptions,
      )
      const org3 = await OrganizationRepository.create(
        {
          displayName: 'hooli',
          identities: [{ name: 'hooli', url: 'https://hooli.com', platform: 'gitmesh' }],
        },
        mockIRepositoryOptions,
      )

...
describe('signalsContentRepository tests', () => {
  ...
  describe('create method', () => {
    it('Should create a content succesfully', async () => {
      ...
      const created = await SignalsContentRepository.create(content, mockIRepositoryOptions)
  ...
  */
  ...
  describe('findAndCountAll method', () => {
    it('Should find signals contant, various cases', async () => {
      ...
      await SignalsContentRepository.create(
        {
          platform: 'reddit',
      ...
      let c2 = await SignalsContentRepository.create(
        {
          platform: 'hackernews',
      ...
      // add bookmark action
      await SignalsActionRepository.createActionForContent(
        {
      ...
      c2 = await SignalsContentRepository.findById(c2.id, mockIRepositoryOptions)

      // another content with a thumbs-up(user1) and a bookmark(user2) action
      let c3 = await SignalsContentRepository.create(
        {
          platform: 'devto',
      ...
      await SignalsActionRepository.createActionForContent(
        {
          type: SignalsActionType.THUMBS_UP,
      ...
      c3 = await SignalsContentRepository.findById(c3.id, mockIRepositoryOptions)

      // filter by action type
      let res = await SignalsContentRepository.findAndCountAll(
        {
          advancedFilter: {
            action: {
              type: SignalsActionType.BOOKMARK,
      ...
      // filter by actionBy
      res = await SignalsContentRepository.findAndCountAll(
        {
          advancedFilter: {
            action: {
              actionById: user2.id,

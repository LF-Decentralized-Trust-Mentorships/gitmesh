...
def api(docker_services):
    ...
    docker_services.wait_until_responsive(
        timeout=300.0, pause=0.1, check=lambda: is_db_up("postgresql://postgres:example@localhost:5433/gitmesh-web")
    )
    tenant_id = "f5c97d75-b919-4be6-9e57-b851efb336a1"
    try:
        api = Repository(tenant_id, "postgresql://postgres:example@localhost:5433/gitmesh-web")
        yield api
    finally:
        api.session.close()

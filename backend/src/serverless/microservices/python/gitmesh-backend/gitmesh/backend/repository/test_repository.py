from gitmesh.backend.models.integration import Integration
from gitmesh.backend.repository import Repository
from gitmesh.backend.models.activity import Activity
from gitmesh.backend.models.member import Member
import uuid
...
def test_find_by_id(api: "Repository"):
    ...
    assert result.username == {"github": "0x161e-swei", "gitmeshUsername": "0x161e-swei"}
...
def test_find_members(api: "Repository"):
    ...
    assert len(result) > 0
    assert type(result[0]) == Member
    assert result[0].username["gitmeshUsername"] == "ThomasPluck"


def test_count(api: "Repository"):
    """Tests the count function for Members that have their slack timzezone in Europe/Amsterdam"""
    result = api.count(Member, {"gitmeshInfo.slack.timezone": "Europe/Amsterdam"})
    assert result == 2
...
def test_find_integration_by_platform(api: "Repository"):
    """ "Tests finding integration by gitmesh platform"""
    result = api.find_integration_by_platform("gitmesh")

    assert type(result) == Integration
    assert result.platform == "gitmesh"

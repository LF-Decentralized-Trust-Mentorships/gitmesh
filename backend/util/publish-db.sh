...
then
    ...
    echo 'localhost:5432:gitmesh-web:postgres:example' > ~/.pgpass && chmod 600 ~/.pgpass && pg_dump --host localhost --port 5432 --username postgres --format plain gitmesh-web -f staging_dump
    echo 'Dump complete! \n Building the docker image out of dump...'
    docker build -f ./Dockerfile-db-latest -t anilbostanci/gitmesh-db-staging:$1 .
    docker push anilbostanci/gitmesh-db-staging:$1
    rm staging_dump
else

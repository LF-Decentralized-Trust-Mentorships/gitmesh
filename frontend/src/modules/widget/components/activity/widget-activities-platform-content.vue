      ...
      <el-collapse-item :name="platform">
        <template #title>
          <div class="flex items-center gap-4">
            <i
              class="ri-arrow-down-s-line text-zinc-400 text-lg"
              :class="{
            ...
            <img
              ...
              class="w-5 h-5"
            />
            <i v-else class="ri-radar-line text-base text-zinc-200" />
            <div class="text-zinc-100 text-xs font-medium">
              {{ value.name }}
            </div>
          </div>
        ...
        <div
          ...
          :key="activity.type"
          class="ml-21 border-b border-t-0 border-zinc-700 h-14 flex items-center justify-between pr-3"
        >
          <div class="text-xs">
            {{ activity.name }}
          </div>
...
const resultSetData = computed(() => {
  ...
  const data = pivot.reduce((acc, el) => {
    ...
    if (!GitmeshIntegrations.getConfig(platform)) {
      return {
        ...acc,
        [platform]: {
          name: platform,
    ...
    return {
      ...acc,
      [platform]: {
        name: GitmeshIntegrations.getConfig(platform).name,
        activities,
        total,
        color: GitmeshIntegrations.getConfig(platform).chartColor,
        image: GitmeshIntegrations.getConfig(platform).image,
      },
    };
  }, {});
...
.el-collapse.custom-collapse-reports {
  ...
  .el-collapse-item__header {
    @apply flex justify-between h-14 items-center bg-zinc-900 hover:bg-zinc-800 hover:cursor-pointer px-3 border-b border-zinc-700;

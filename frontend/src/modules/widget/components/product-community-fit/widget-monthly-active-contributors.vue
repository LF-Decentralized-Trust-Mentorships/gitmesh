<template>
  <query-renderer v-if="cubejsApi" :cubejs-api="cubejsApi" :query="query">
    <template #default="{ resultSet, loading, error }">
      <div class="bg-zinc-900 px-6 py-5 rounded-lg shadow">
        <!-- Widget Header -->
        <div
          class="flex grow justify-between items-center pb-5 border-b border-zinc-700"
...
const widgetChartOptions = computed(() => chartOptions('area', {
  ...
  annotationPlugin: {
    annotations: {
      idealRange: {
        backgroundColor: 'rgba(253, 246, 245, 0.1)',
        yMin: 50,
        yMax: 100,
...
const datasets = computed(() => [
  ...
  },
  {
    ...
    backgroundColor: 'rgba(250, 237, 234, 0.1)',
    borderColor: 'transparent',
    pointStyle: 'rect',
    hidden: true,
  },

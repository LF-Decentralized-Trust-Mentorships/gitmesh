...
export default {
  ...
  state: () => ({
    ...
    loading: false,
    cubejsToken: null,
    cubejsTokenAttempted: false,
  }),

  getters: {
    cubejsToken: (state) => state.cubejsToken,
    cubejsTokenAttempted: (state) => state.cubejsTokenAttempted,
  ...
  mutations: {
    RESETED(state) {
      ...
      state.cubejsTokenAttempted = false;
    },
    ...
    GET_CUBE_TOKEN_SUCCESS(state, token) {
      ...
      state.cubejsTokenAttempted = true;
    },

    GET_CUBE_TOKEN_ERROR(state) {
      state.cubejsTokenAttempted = true;
    },
  },
  ...
  actions: {
    ...
    async getCubeToken({ commit }) {
      try {
        ...
        // Silently fail if CubeJS is not available (e.g. in development)
        // This is expected when the cubejs module fails to load in the backend
        console.warn('CubeJS token could not be retrieved. Analytics features may be limited.', error);

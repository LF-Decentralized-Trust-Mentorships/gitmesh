    ...
    <div class="max-h-[40rem] overflow-auto">
        ...
        <div class="p-2">
          ...
          <div v-if="currentTenant.id === tenant.id" class="pt-1 -mx-1">
            ...
            <div v-if="hasPermissionsForSettings" class="px-1">
              <div class="p-3 h-10 text-xs text-white mt-1 rounded hover:bg-zinc-900 cursor-pointer" @click.stop="emit('edit', tenant)">
                Edit workspace
              </div>
            </div>
          </div>
      ...
      >
        <div
          ...
          class="px-3 h-10 text-sm font-normal rounded hover:bg-zinc-900 cursor-pointer flex items-center text-brand-500"
          @click="emit('add')"
        >
    ...
    <section class="px-2 pb-2 pt-1">
      ...
      <div class="p-3 flex items-center">
        <app-avatar
          ...
          size="xxs"
        />
        <div class="pl-3 font-medium leading-5 text-2xs text-white">
          {{ currentUser.email }}
        </div>
      </div>
      ...
      <router-link
        :to="{ name: 'editProfile' }"
        class="px-3 h-10 text-sm font-normal rounded hover:bg-zinc-900 cursor-pointer flex items-center text-white"
      >
        <i class="ri-account-circle-line text-base text-zinc-400 mr-3" />
      ...
      <div
        ...
        @click="doSignout()"
      >
        <i class="ri-logout-box-r-line text-base text-zinc-400 mr-3" />
        <span>Sign out</span>
      </div>
    </section>
  </div>

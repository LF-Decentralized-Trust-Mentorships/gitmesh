<template>
  <div>
    <el-popover
      ...
      :width="340"
      trigger="click"
      popper-class="!p-0 transform translate-x-1 !bg-zinc-900 !border-zinc-700"
    >
      <template #reference>
        <cr-menu-workspace-card
          class="h-14 hover:bg-zinc-800"
          :class="[isDropdownOpen ? '!bg-zinc-800' : '', props.collapsed ? 'px-2 justify-center' : 'pl-4 pr-3.5']"
          :tenant="currentTenant"
          :collapsed="props.collapsed"
        >
          <i class="ri-settings-3-line text-lg text-zinc-400 transform -translate-x-0.5 block mr-1.5" />
        </cr-menu-workspace-card>
      </template>
      <div>

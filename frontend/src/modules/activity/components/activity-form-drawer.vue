...
onActivated(() => {
  // Small delay to ensure we don't block legitimate close events during activation
  setTimeout(() => {
...
const isVisible = computed({
  ...
  set(value) {
    // If we are deactivated (tab switch), ignore the close event from el-drawer
    if (isDeactivated.value && !value) {
      return;
    }
    if (!value) {
      reset();

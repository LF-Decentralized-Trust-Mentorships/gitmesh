<template>
  <div class="flex items-center gap-3">
    <div>
      <div
        class="min-h-8 min-w-8 w-8 h-8 border border-zinc-700 rounded flex items-center justify-center overflow-hidden"
        :class="{
          'bg-zinc-900': organization.logo,
          'bg-zinc-800': !organization.logo,
        }"
      >
        <app-avatar-image
          ...
          class="max-h-8"
        >
          <i class="ri-community-line text-lg text-zinc-200 h-5" />
        </app-avatar-image>
      </div>
    </div>
    <div class="overflow-hidden mr-6">
      ...
      >
        <div
          ref="nameRef"
          class="font-semibold text-sm text-zinc-100 overflow-hidden whitespace-nowrap text-ellipsis truncate"

      ...
      <div class="-mx-6 mt-6">
        <template v-if="props.organization.identities && props.organization.identities.length > 0">
          <a
            ...
            :class="getIdentityLink(identity) ? ' hover:bg-zinc-800 cursor-pointer' : ''"
        ...
        <el-divider
          v-if="showDivider"
          class="border-t-zinc-700"
        />
        <a
          v-for="email of organization.emails"
          :key="email"
          class="px-6 py-2 flex justify-between items-center relative hover:bg-zinc-800 transition-colors cursor-pointer"
          :href="`mailto:${email}`"
          target="_blank"
          rel="noopener noreferrer"
        >
        ...
        <div
          v-if="noIdentities"
          class="text-sm text-zinc-400 px-6 pt-2"
        >
          <router-link
            :to="{

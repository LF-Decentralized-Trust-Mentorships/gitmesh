      ...
      >
        <template #button>
          <div class="relative">
            ...
            <div
              ...
              class="w-2 h-2 rounded-full bg-brand-500 outline outline-4 outline-zinc-900 absolute top-[-4px] right-[-4px]"
            />
          </div>
        </template>
        <template #optionPrefix="{ item }">
          <img
            ...
            class="w-4 h-4 mr-2"
          />
          <i v-else class="ri-radar-line text-base !text-zinc-200 !mr-2" />
        </template>
      </app-filter-list-item>
...
const platformOptions = (platform) => GitmeshIntegrations.getConfig(platform);
...
<style lang="scss">
.custom-btn {
  @apply bg-zinc-900 border border-zinc-700 shadow text-zinc-100;
...
.switch-filter .el-switch__label span {
  @apply text-xs text-zinc-100 font-normal;
}
</style>

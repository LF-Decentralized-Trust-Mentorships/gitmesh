<template>
  <div class="grid gap-x-12 grid-cols-4">
    ...
    <div
      class="col-span-3"
    >
      <div class="flex  border-b border-zinc-700 h-8 items-center">
        <div class="flex items-center flex-grow">
          <span
            class="uppercase text-zinc-400 text-2xs font-semibold tracking-wide w-1/3"
          >Organization</span>
          <span
            class="uppercase text-zinc-400 text-2xs font-semibold tracking-wide w-1/3"
          >Job Title</span>
          <span
            class="uppercase text-zinc-400 text-2xs font-semibold tracking-wide grow w-1/3"
          >Period</span>
        </div>
        <div class="w-18" />
      </div>

      <div v-for="(organization, oi) in organizations" :key="organization.id" class="flex gap-3 border-b border-zinc-700 h-8 items-center min-h-17">
        ...
        <div class="flex items-center">
          <div class="cursor-pointer h-8 w-8 mr-2 group" @click="edit(oi)">
            <span class="ri-pencil-line text-lg text-zinc-400 group-hover:text-zinc-200" />
          </div>
          <div class="cursor-pointer h-8 w-8 group" @click="organizations.splice(oi, 1)">
            <span class="ri-delete-bin-line text-lg text-zinc-400 group-hover:text-zinc-200" />
          </div>
        </div>
      </div>

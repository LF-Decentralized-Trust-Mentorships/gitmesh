<template>
  <section v-if="props.loading">
    <div
      ...
      :class="{ 'bg-zinc-900': props.isPrimary }"
    >
  ...
  <section v-else class="h-full">
    ...
    >
      ...
      <div class="h-13 flex justify-between items-start">
        ...
        >
          <span class="ri-arrow-left-right-fill text-base text-zinc-400 mr-2" />
      ...
      <div class="pb-4">
        <h6
          class="text-base text-zinc-100 font-semibold"
          v-html="$sanitize(member.displayName)"
        />
        <div
          v-if="member.attributes.bio?.default"
          ref="bio"
          class="text-zinc-400 leading-5 !text-xs merge-member-bio"
      ...
      <div>
        <article
          class="flex items-center justify-between h-12 border-b border-zinc-700"
        >
          <p class="text-2xs font-medium text-zinc-200 pr-4">
            Engagement level
          </p>
        ...
        >
          ...
          <p class="text-xs text-zinc-300 text-right">
            {{ member.attributes.location?.default || '-' }}
          </p>
        </article>
        <article
        ...
        >
          ...
          <app-tags
            ...
            :member="member"
            :editable="false"
            tag-classes="!bg-zinc-800 !text-xs !leading-5 !py-0.5 !px-1.5"
          />
          <span v-else>-</span>
        </article>
      </div>
      <div class="pt-5">
        ...
        >
          <a
            ...
            class="pb-2 pt-3 flex items-center text-zinc-300 hover:text-zinc-100"
            target="_blank"
          ...
          >
            ...
            <el-tooltip v-else :content="platform" placement="top">
              <i class="ri-user-3-fill text-zinc-200 text-lg mr-4" />
            </el-tooltip>
            <div
              v-if="
                platform === 'linkedin'
            ...
            >
              *********
              ...
              >
                <i
                  class="ri-lock-line text-zinc-400 ml-2"
                />
              </el-tooltip>
            </div>
            <span
        ...
        <a
          ...
          :key="email"
          :href="`mailto:${email}`"
          class="pb-2 pt-3 flex items-center text-zinc-300"
          target="_blank"
          rel="noopener noreferrer"
        >
          <span
            class="ri-mail-line text-lg text-zinc-400 mr-4 h-5 flex items-center"
          />
...
const platformDetails = (platform) => GitmeshIntegrations.getConfig(platform);
const identityUrl = (platform, username, attributes) => {
  if (platform === 'discord' || platform === 'slack') {
    return null;
  }

  return GitmeshIntegrations.getConfig(platform)?.url({ username, attributes });
};

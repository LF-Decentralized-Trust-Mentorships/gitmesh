  ...
  >
    <div class="member-form-page">
      <el-button
        ...
        link
        :icon="ArrowPrevIcon"
        class="text-gray-300 btn-link--md btn-link--secondary p-0"
        @click="onCancel"
      >
        Contacts
      </el-button>
      ...
      <el-container
        v-if="!isPageLoading"
        class="bg-zinc-900 rounded-lg shadow shadow-black/30"
      >
        <el-main class="p-6">
          <el-form
            ref="formRef"
          ...
          >
            ...
            />
            <el-divider
              class="!mb-6 !mt-8 !border-zinc-700"
            />
            ...
            />
            <el-divider
              class="!mb-6 !mt-16 !border-zinc-700"
            />
...
async function onSubmit() {
  ...
  const data = {
    displayName: formModel.value.displayName || 'Unknown Contact',
    ...formModel.value.emails && {
      emails: formModel.value.emails,
    },
    joinedAt: formModel.value.joinedAt || new Date().toISOString(),
    platform: formModel.value.platform || 'other',
    ...
    ...formModel.value.organizations.length && {
      ...
      organizations: formModel.value.organizations.map(
        (o) => {
          const org = {
            id: o.id,
            name: o.displayName || o.name || o.label,
            title: o.memberOrganizations?.title || null,
            startDate: o.memberOrganizations?.dateStart || null,
            endDate: o.memberOrganizations?.dateEnd || null,
            source: 'ui',
          };
          console.log('Mapping organization for submission:', org);
          return org;
        },
      ).filter(
        (o) => {
          if (!o.id) {
            console.warn('Filtering out organization without ID:', o);
            return false;
          }
          return true;
        },
    ...
    ...(Object.keys(formattedAttributes).length
      ...
      attributes: {
        ...
        // Ensure these flags are set to false for manually created contacts
        // Use 'default' key as that's what the backend queries look for
        isBot: {
          default: false,
        },
        isTeamMember: {
    ...
    username: Object.keys(formModel.value.username).length 
      ? formModel.value.username 
      : {
          [formModel.value.platform || 'other']: formModel.value.displayName 
            || formModel.value.emails?.[0] 
            || `contact-${Date.now()}`,
        },
  ...
  console.log('Creating contact with data:', JSON.stringify(data, null, 2));
  ...
  .personal-details-form {
    ...
    & .app-tags-input {
      & .el-input__wrapper {
        ...
        & .el-tag {
          @apply m-0 h-6 bg-zinc-800 border-zinc-700;
        }
      }
    }
  }

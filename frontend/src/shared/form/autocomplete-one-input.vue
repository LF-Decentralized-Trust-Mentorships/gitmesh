  ...
  methods: {
    async onChange(value) {
      ...
      console.log('autocomplete onChange called:', {
        ...
        createIfNotFound: this.createIfNotFound,
        typeofQuery: typeof query,
        queryNotEmpty: query !== '',
        notValue: !value,
        willCreate: typeof query === 'string' && query !== '' && this.createIfNotFound && !value
      });
      ...
      if (
        ...
        console.log('Creating new item with query:', query);
        const newItem = await this.createFn(query);
        console.log('createFn returned:', newItem);
        this.localOptions.push(newItem);
        this.$emit('update:modelValue', newItem);
      } else {
        console.log('Using else branch, emitting:', {
          ...value,
          ...this.storeKey && {

<template>
  <div :class="filterClass">
    ...
    >
      ...
      <div
        class="border-t border-zinc-700 flex items-center justify-between -mx-2 px-4 pt-3 pb-1"
      >
        <el-button
...
.filter-list-item {
  @apply text-xs mb-4;
  &-btn.el-button {
    @apply h-8 flex items-center p-2 bg-zinc-900 border border-zinc-700 outline-none text-zinc-400 text-xs;
    ...
    &:focus,
    &:visited {
      @apply bg-zinc-800 outline-none text-zinc-400 border-zinc-700;
    }

    &.is-active {
      @apply bg-brand-900 border-brand-800 text-brand-100 outline-none;
      &:hover {
        @apply bg-brand-800;
      }
    }
  ...
  &-btn__close.el-button {
    @apply w-8 h-8 flex items-center p-2 text-zinc-400;
  }

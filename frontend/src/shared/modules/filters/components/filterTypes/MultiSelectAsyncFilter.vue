        ...
        >
          ...
          <template v-if="config.id === 'organizations'">
            <span class="flex items-center justify-center w-6 h-6 p-1 mr-3 border rounded-md ">
              ...
              <i v-else class="flex items-center justify-center w-4 h-4 text-zinc-200 ri-community-line" />
            </span>
          </template>
          {{ option.label }}
        </el-option>
  ...
  .el-input__wrapper:hover {
    ...
    input {
      ...
      &:focus {
        ...
        @apply bg-zinc-800 shadow-none outline-none h-full min-h-8;
      }
    }
  }
...
.filter-multiselect-popper {
  ...
  .el-select-dropdown{
    @apply -mx-4 p-0 mt-3 border-t border-zinc-700;

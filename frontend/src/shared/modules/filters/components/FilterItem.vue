<template>
  <div class="flex items-center shadow rounded-md">
    ...
    >
      <template #reference>
        <div
          class="border border-zinc-700 rounded-l-md h-8 flex items-center py-1 px-2 bg-black cursor-pointer hover:bg-zinc-900 transition"
          :class="[{ '!bg-zinc-900': isOpen }, props.chipClasses]"
          data-qa="filter-list-chip"
        >
          <i class="text-base text-white mr-2" :class="config.pngnClass" />
          <span
            class="text-xs text-zinc-400 filter-item-text leading-6"
            v-html="$sanitize(
              (props.modelValue && config.itemLabelRenderer(props.modelValue, props.config.options, data))
                || `<span class='!text-zinc-200'>${config.label}...</span>`,
            )"
          />
        </div>
      </template>
    ...
    <div
      ...
      class="border border-zinc-700 rounded-r-md h-8 flex items-center p-2 bg-black -ml-px hover:bg-zinc-900 transition cursor-pointer group"
      data-qa="filter-list-chip-close"
      @click="emit('remove')"
    >
      <span class="ri-close-line text-base flex items-center h-4 text-zinc-200 group-hover:text-white" />
    </div>
  </div>
</template>
...
.filter-item-text{
  b{
    @apply font-medium text-white mr-2;
  }
}
</style>

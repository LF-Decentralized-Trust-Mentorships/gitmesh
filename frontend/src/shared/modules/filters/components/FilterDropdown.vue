  ...
  <el-popover v-model:visible="open" placement="bottom-end" size="large" width="20rem" popper-class="!p-0" trigger="click">
    ...
    <div class="max-h-80 overflow-auto px-2 py-3">
      <article
        ...
        :key="key"
        class="mb-1 p-3 rounded flex justify-between items-center transition whitespace-nowrap h-10 hover:bg-zinc-900 text-xs"
        :class="isSelected(key) ? 'bg-zinc-900 text-zinc-200' : 'text-white cursor-pointer'"
        data-qa="filter-list-item"
        @click="add(key)"
      >
        <span><i :class="iconClass" class="text-base text-zinc-200 mr-3" />{{ label }}</span>
        <i :class="isSelected(key) ? 'opacity-100' : 'opacity-0'" class="ri-check-line !text-zinc-200 !mr-0 ml-1" />
      </article>

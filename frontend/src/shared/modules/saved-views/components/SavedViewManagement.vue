  ...
  <div class="px-2">
    <div class="py-2">
      <article class="py-2.5 px-3 flex justify-between">
        ...
        <p class="text-xs text-zinc-200 italic">
          default view
        </p>
      </article>
    </div>
    ...
    <div class="py-2">
      <vue-draggable-next :list="views" @change="onListChange">
        <article
          v-for="view of views"
          :key="view.id"
          class="p-2 rounded flex items-center justify-between flex-grow transition hover:bg-zinc-900 cursor-grab"
        >
          <div class="flex items-center">
            <i class="ri-draggable text-base text-zinc-200 mr-2" />
            <span class="text-sm leading-5 text-white">{{ view.name }}</span>
          </div>
          <div class="flex items-center">
            <el-tooltip placement="top" content="Edit view">
              <div
                class="h-6 w-6 flex items-center justify-center ml-1 group cursor-pointer hover:bg-zinc-800 rounded"
                @click="edit(view)"
              >
                <i class="ri-pencil-line text-sm text-zinc-200 group-hover:text-zinc-400" />
              </div>
            </el-tooltip>
            <el-tooltip placement="top" content="Duplicate view">
              ...
              >
                <i class="ri-file-copy-line text-sm text-zinc-200 group-hover:text-zinc-400" />
              </div>
            </el-tooltip>
            <el-tooltip placement="top" content="Delete view">
              <div
                ...
                @click="remove(view)"
              >
                <i class="ri-delete-bin-line text-sm text-zinc-200 group-hover:text-zinc-400" />
              </div>
            </el-tooltip>
          </div>
        </article>

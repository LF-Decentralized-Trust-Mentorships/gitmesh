...
const imageProperties = computed(() => GitmeshIntegrations.getConfig(props.platform));
const href = computed(() => {
  if (props.href) {
    return props.href;
  }
  ...
  return (props.usernameHandles.length === 1 || props.attributes?.url?.[props.platform]
    ? (GitmeshIntegrations.getConfig(props.platform)?.url({ username: props.usernameHandles[0], attributes: props.attributes }) ?? props.backupUrl)
    : null);
});

  ...
  static handle(error) {
    ...
    if (selectErrorCode(error) === 500) {
      ...
      if (errorMsg.includes('cannot be null')) {
        Message.error('Required field is missing. Please try again.');
      } else {
    ...
    if (selectErrorCode(error) === 542) {
      Message.error(
        'An error has occurred setting up the integration, please reach out to us via chat, or via email (support@gitmesh.dev)',
        { duration: 0 },
      );
      return;
    }
